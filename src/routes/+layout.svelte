		...
		<div class="mx-auto flex max-w-[640px]">
			{#each tabs as tab (tab.href)}
				...
				<a
					href={tab.href}
					class:active={active}
					class="flex flex-1 touch-manipulation select-none flex-col items-center gap-1 pt-2 text-muted-foreground no-underline transition-[color,transform] active:scale-95 hover:text-foreground"
				>
